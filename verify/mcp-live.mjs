...
test(`MCP tool call through the harness (${MODEL})`, { timeout: 180000 }, async () => {
  ...
  try {
    ...
    srv = spawn("codex", ["app-server", ...cfg], { env: { ...process.env, BRIDGE_KEY: "sk-spike-local", CODEX_HOME: codexHome }, stdio: ["pipe","pipe","ignore"] });
    ...
    const ts = await send("thread/start", { model: MODEL, modelProvider: "gemini", cwd: WS, approvalPolicy: "never", sandbox: "workspace-write" });
